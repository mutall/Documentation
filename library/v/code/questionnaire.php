    ...
    class questionnaire extends schema {
        ...
        */  
        function load(string $logfile="log.xml", string $error_file="errors.html")/*:questionnaire.Imala*/ {
            //
            //Create the requested file for logging progress
            $log = (log::$current = new \log($logfile));
            //
            //Open the text file stream for logging errors
            $this->error_file = $error_file;
            \questionnaire::$current->error_stream = fopen($error_file, "w");
            //
            //Compile the inputs (used to crate this questionnaire) to produce 
            //active milk tables and artefacts
            ...
            //the look of the (sml) logfile. NB. Dependency is a function of the
        ...
        //Load common is an extemsion of the basic load process but returns
        //a html report
        ...
        function load_user_inputs():array/*library.Imala*/{
            ...
            return self::get_crud_result($Imala);
        }
        ...
        // 
        //The following is the shape of the Imala, the CRUD version:-
        //Imala = syntax|runtime
        //syntax = {class_name:"syntax", errors:Array<string>}
        //runtime = {class_name:"runtime", result:Array<error|col>}
        //where:-
        //error = ['error', string]
        //col = [cname, autonumber, friend]
        static function  get_crud_result(array /*quest.Imala*/$result):array/*libray.Imala*/{
            //
            //Return the syntax error as it is:
            ...
            //
            //Convert the labels to an array of crud outputs. There will be 2 
            //types of crud outputs: An error or a (primary or foreign) key.
            //
            //Start with an empty crud output
            $output= [];
            //
            //Loop through all the labels to collect the Crud outputs
            foreach($labels as $label){
                //
                //Destructure the label
                list($dbname, $ename, $alias, $cname, $ans) = $label;
                //
                //If the answer is an error, add it to the output
                if ($ans instanceof myerror){ 
                    $output += ['error', $ans];
                    continue;
                } 
                //Get the column that matches the label
                $col = $this->open_dbase($dbname)->entities[$ename]->columns[$cname];
                //
                //If the column is either primary or foreign we need the friendly
                //component
                if ($col instanceof \primary || $col instanceof \foreign){
                    //
                    //Get the friend
                    $friend = $col->away()->get_friend($ans->value);
                    //
                    //Compile the output
                    $output += ['pk', $ans, $friend];
                }
                //
                //Ignore attribute columns that are not erroneous
            }
            //
            //Return the rutime result
            return ['class_name'=>'runtime', 'result'=>$output];    
        }
        ...
        private function get_summary(array $result):string{
            ...
            $msg = self::summarise_labels("Table-independent artefacts", $result['labels']);
            //
            ...
            if (count($tables)>0)
                $msg .= "Table-dependent artefacts<br/>"
                    .implode("<br/><br/>",
                        array_map(
                            fn($table)=>self::summarize_labels("Summary for table ".$table['name'], $table['labels']),
                            $tables
                        )     
                    );
            //
        ...
        //Summarise the given labels. A labele destructures to:-
        //[dbname, ename, alias, cname, answer]
        static function summarise_labels(string $header,array $labels):string{
            //
            //Isolate the erroneous labels
         ...
         */
        private function export_ti_artefacts(): array {
            //
        ...
        private function export_td_artefacts(): array/* <{tname, errors, rows}> */ {
            //
            //Prepare to return the reult
            $result/* :Array<{tname, answer, rows}> */ = [];
            //
            //This process (unlike the table independent version) is driven by 
            //the collection of (milk) tables.
            foreach ($this->tables as $table) {
                //
                //Export the tables data. NB: save requires a row of data; its 
                //null by default
                $ans = $table->save(null);
                //
                $result[]=[
        ...
        * aliases
        * @param null|row row For milk table dependent cases, this is row being 
        * processed; otherwise it is null
        * @return array<label>
        */
        public function export_artefacts(array $artefacts, $row): array{
            //
            //For each save mode (for cross and non-cross members)...
            foreach ([false, true] as $cross_member) {
                ...
                foreach ($artefacts as $artefact) {
                    ...
                    //
                    //Insert or update the artefact, ignorring teh results until
                    //after all the saves
                    $artefact->save($row);
                    //
                    ...
                    if ($cross_member)
                        //
                        //...again do the update an dignore the result
...
//than a property.  
    class artefact extends \table {
        ...
        function yield_labels():\Generator{
            //
            //Convert every column of this artefact to a label
            foreach($this->columns as $column){
                //
                //Pass any location details from the original expression
                //to the current answer, if any.
                if (isset($column->exp->position)) 
                    $column->answer->position = $column->exp->position;
                //
        ...
        public function save_indices(/* row|null */$row): array/* {condition, exp} */ {
            ...
            //...the primary key column is set...
                    ...
                    //... with a scalar value (not a null for new entries)...
                    && ($pk = $this->pk()->exp) instanceof scalar

                    //...that has a valid (autonumber) value (not just the position info)
                    & !is_null($pk->value)
            )
    ...
    class column extends \column {
        ...
        private function parse_default(string $default): expression {
            ...
            switch ($default) {
                ...
                //closing parentheses and are in upper case. Translate all such
                //cases as missig data
                case 'CURRENT_TIMESTAMP':
                    return new myerror(
                            "Unable to evaluate function $default. So missingg data"
                    );
                //
        ...
        //Writing the primary key column, which effectively, writes the whole record
        //of underlying artefact.
        //For a table independent situation, the row is nul; otherwise it is the 
        //row (index) of the record being written.
        //The return value is an answer, i.e., myerror, or scalar -- the expression 
        //resulting saving a record.
...
//Modelling the tabular data layout, a.k.a., milk. It in 
//logging
    abstract class table extends \schema {
        ...
        private function write_body(array $artefacts):ans {
            //
            //Its a sign of an error if there are no artfacts to write
            ...
            foreach ($this->read() as $Ibarrel) {
                ...
                //
                //Check whether the row of data, Ibarrel, is clean or not
                //Ignore that row if it is dirty, i.e., no clean
            ...
            //
            //Return this tables error count
...
//Modelling an sql statement as a source of body rows
...
//copies of this instance in order to ssave memory for large data loads.  
    class barrel extends \schema {
        ...
        //Compile the results of this barrel
        function compile_result(){
            //
            //Loop through all teh artefacts of this berrel
            $labels = array_map(
    ...
    abstract class expression extends \mutall implements \operand {
        ...
        public /*[rowIndex, colIndex?]|null*/ $position = null; 
        //
        function __construct(){}
...
//Modelling prepared and ordinary sql staments for capturing data
    abstract class statement {
        ...
        //Execute a statement using a prepared statement to return an 
        //expression as the result. The prepared version is valid
        //only when all the columns bound to parameters of this statment are
        //valid. Typically the result is a primary key value, error, or null
        function execute(): ans {
            ...
            //
            //The bound columns are fewer than expected. Somethhing must be wrong!
            if ($all_valids < count($this->columns)) {
                //
                //Report this unusual situation
                //
                //Expected of bound column names
                $expecteds = array_map(fn($col) => "$col->source", $this->columns);
                //
                //Actual bound columns
                $actuals = array_map(fn($col) => "$col->source", $cols);
                //
                return new myerror(
                        "Expected "
                        . count($this->columns)
                        . " bound columns, " . json_encode($expecteds)
                        . " but found only $all_valids, i.e., " . json_encode($actuals)
                );
            }
            //
            //Use the prepared statement to execute this statement. Prepare to catch
            //The execute method seed to throw such a hard exception the that
            //a local try/catch block could not catch it. I did not understand why:
            //hence this new version
            //
            //Save the current error mode attribute of this statement's pdo
            $mode = $this->pdo->getAttribute(\PDO::ATTR_ERRMODE);
            //
            //Change it to silent mode, so that errors in execute do not throw 
            //exception
            $this->pdo->setAttribute(\PDO::ATTR_ERRMODE, \PDO::ERRMODE_SILENT);
            //
            //Execute the prepared statement and return ok when successful
            $ok = $this->handle->execute();
            //
            //Restore the last mode saved
        ...
        function handle_fewer(): ans {
            ...
            $dbase = $this->artefact->source->open_dbase($this->artefact->source->dbname);
...
//data capture columns that are not cross members
    class insert extends statement {
        ...
        function handle_success(\PDOStatement $stmt): ans {
            ...
            $pk = $this->artefact->get_dbase()->lastInsertId();
            //
...
//database
    class index extends \schema {
        ...
        //new record or update an exiisting one.
        function write(/* row|null */$row): ans {
            //
            ...
            //takes one of 3 forms:null_, scalar or myerror
            $result = $this->select->execute();
            //
            //If a record was retrieved oe there was an error then we return the 
            //result...
            if (!($result instanceof \null_))
                return $result;
            //
            //...otherwise we execute the insert statement. NB: the resulting scalar 
            //will be marked with the insert type.
