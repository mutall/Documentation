...
class database extends schema {
  ...
  get_roles(): Array<{ key: string; value: string }> {
    ...
    let roles: Array<{ key: string; value: string }>;
    //
    //
    const user_entities = Object.values(this.entities).filter((Entity) => {
...
class foreign extends column {
  ...
  get_type() {
    ...
    if (
      this.static_column.comment.type === undefined ||
      this.static_column.comment.type === null
    ) {
      //
...
class attribute extends column {
  //
  // we suspect that this was added to suport
  // enumerated types
  public type?: string;
  //
