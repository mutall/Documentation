...
export class theme extends outlook.panel {
  ...
  protected initialize_styles(col_names: Array<string>) {
    ...
    const sheet: CSSStyleSheet = (<HTMLStyleElement>this.get_element("columns"))
      .sheet!;
    //
  ...
  private select_column(evt: Event | HTMLTableCellElement) {
    ...
    const th =
      evt instanceof HTMLTableCellElement
        ? evt
        : <HTMLTableCellElement>evt.target;
    //
    ...
    if (selected_column !== null) {
      //
      //2.2.1 Get its index.
      const index = (<HTMLTableCellElement>selected_column).cellIndex;
      //
  ...
  private async execute_outcome(outcome: outcome, request: offset) {
    ...
    switch (outcome.type) {
      ...
      case "fresh":
        ...
        if (tbody === null) throw new schema.mutall_error("tbody not found");
        //
  ...
  protected async load_body(
    offset: offset /*:int*/,
    dir: direction /*:mytop | bottom*/
  ) {
    //
    ...
    const constrained_limit =
      h < app.app.current.config.limit ? h : app.app.current.config.limit;
    //
    //Query the database
  ...
  private within_joint(request: offset): boolean {
    ...
    const condition =
      ...
      request >= this.get_joint("top") && request < this.get_joint("bottom");
    return condition;
  }
  //
  ...
  private within_extreme(request: offset): boolean {
    ...
    return request >= this.extreme.top && request < this.extreme.bottom;
  }
  //
  ...
  private within_view(req: offset): boolean {
    ...
    return (
      true && //true is for appeasing the IDE.
      //
      //...the top view is set...
      this.view.top !== null &&
      //
  ...
  private get_joint(dir: direction /*top|bottom*/): offset {
    ...
    return this.within_extreme(raw_boundary) ? raw_boundary : this.extreme[dir];
  }
  //
  //
  ...
  public load_tr_element(
    ...
    const pairs: Array<[string, library.basic_value]> =
      row === undefined
  ...
  private get_outcome(request: offset): outcome {
    ...
    if (this.within_view(request)) return <nothing>{ type: "nothing" };
    //
    ...
    if (this.within_joint(request)) {
      ...
      const dir = request < this.view.top ? "top" : "bottom";
      //
      ...
      const start_from = dir === "top" ? this.get_joint(dir) : this.view[dir];
      //
    ...
    if (this.within_extreme(request)) {
      ...
      const view_top = request < this.extreme.top ? this.extreme.top : request;
      //
      ...
      const view_bottom = y > this.extreme.bottom ? this.extreme.bottom : y;
