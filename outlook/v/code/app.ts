...
export abstract class app extends outlook.view {
  ...
  async initialize(): Promise<void> {
    ...
    this.panels.set(
      "services",
      <outlook.panel>new services(<outlook.view>this)
    );
    //
    //Set the theme panel
    this.panels.set(
      "theme",
      new theme.theme(this.subject, "#content", <outlook.view>this)
    );
    //
  ...
  async set_dbase() {
    ...
    const idbase = await server.exec(
      "database",
      [this.dbname],
      "export_structure",
      []
  ...
  async login(User?: outlook.user) {
    ...
    const ids = <Array<{ id: string }>>(
      await server.exec("database", ["mutall_users"], "get_sql_data", [sql])
    );
    //
  ...
  async register(): Promise<Array<string> | undefined> {
    ...
    const Role = new outlook.choices<string>(
      this.config.general,
      inputs,
      "role_id"
    );
  ...
  async relink_user(): Promise<void> {
    ...
    const ok: boolean = await server.exec("tracker", [], "relink_user", [
      links,
    ]);
    //
  ...
  private collect_broken_replicas(): Array<replica> {
    ...
    const replicas: Array<replica> = role.map((role) => {
      return { ename: role.key, cname: "user" };
    });
    //
  ...
  async edit_table() {
    ...
    //entity name
    const Choice = new outlook.choices<library.ename>(
      this.config.general,
      pairs,
      "table",
      null,
...
export class services extends outlook.panel {
  ...
  async continue_paint() {
    ...
    const prods =
      this.products === null ? (<app>this.base).products : this.products;
    //
    //
    ...
    Object.keys(product!.solutions).forEach((id) => {
      ...
      switch (sol.listener[0]) {
        ...
        case "crud":
          ...
          const subject = <outlook.subject>[
            ename,
            dbname === undefined ? app.current.dbname : dbname,
          ];
          //
          //
