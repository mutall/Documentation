...
export function create_io(
  ...
  if (col.read_only !== undefined && col.read_only) return new readonly(anchor);
  //
  //Characterize the foreign and primary key columns
  if (col instanceof schema.primary) return new primary(anchor);
  if (col instanceof schema.foreign) return new foreign(anchor);
  //
  ...
  if (col.length === 1) return new checkbox(anchor);
  //
  ...
  if (["timestamp", "date", "time"].find((dtype) => dtype === col.data_type))
  ...
  //remote serve.
  if (
    ["logo", "picture", "profile", "image", "photo"].find(
      (cname) => cname === col.name
    )
  )
    return new file(anchor, "image");
  //
  if (col.name === ("filename" || "file")) return new file(anchor, "file");
  //
  //URL
  ...
  if (["varchar", "text"].find((dtype) => dtype === col.data_type))
...
export abstract class io {
  ...
  public show_label(
    ...
    const header =
      text instanceof HTMLElement ? text : this.document.createTextNode(text);
    ...
    for (let name in this) {
      ...
      if (id === undefined || id === null)
        throw new schema.mutall_error(
          `This property ${name} points to an unidentified element`
        );
      //
...
export class readonly extends io {
  ...
  constructor(anchor: HTMLElement) {
    ...
    this.output = this.create_element(anchor, `span`, {
      className: "read_only",
    });
  }
  //
...
export class foreign extends io {
  ...
  constructor(anchor: HTMLElement) {
    ...
    this.friendly = this.create_element(anchor, `span`, {
      className: "normal friendly",
    });
    //
  ...
  set input_value(i) {
    ...
    if (typeof i === "string") {
      ...
      if (pk === undefined || friend === undefined) {
        throw new schema.mutall_error(
          `THe foreign key value '${i}' is not correctly formatted`
        );
      }
      //
      //Set the button's
...
export class file extends input {
  ...
  constructor(
    ...
    this.create_element(this.source_selector, "option", {
      value: "local",
      textContent: "Browse local",
    });
    this.create_element(this.source_selector, "option", {
      value: "remote",
      textContent: "Browse remote",
    });
    //
    //
  ...
  public toggle_source(evt: Event): void {
    ...
    const link = <HTMLLinkElement | null>(
      this.document.querySelector("#theme_css")
    );
    ...
    this.input_header!.textContent = `Select ${
      selected === "remote" ? "file" : "folder"
    }`;
  }
  //
  ...
  update_stylesheet(sheet: CSSStyleSheet, selection: String, hide: Boolean) {
    ...
    const index = rules.findIndex(
      (rule1) => rule1.selectorText === `.${selection}`
    );
  ...
  public async browse(
    ...
    const target = this.source_selector.value === "local" ? "folder" : "file";
    //
    ...
    const path = await new tree.browser(
      target,
      url,
      Inode,
      initial
  ...
  set input_value(i: library.basic_value) {
    ...
    if (this.type === "image") {
      ...
      this.image!.src = i === null ? "/pictures/default.jpeg" : String(i);
    }
  }
}
...
export class primary extends io {
  ...
  constructor(anchor: HTMLElement) {
    ...
    //Tag where to reporting  runtime errors that arise from a saving the record
    // (with this primary key to the server)
    this.errors = this.create_element(
      anchor,
      `span`,
      //
  ...
  public see_error(evt: Event): void {
    ...
    (<HTMLElement>evt.target!).textContent = this.errors.hidden
      ? "see error"
      : "close error";
  }
  //
  ...
  get input_value() {
    ...
    const value =
      this.output.textContent === "" ? null : this.output.textContent;
    return value;
  }
  //
